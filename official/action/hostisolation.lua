...
elseif hunt.env.is_windows() then
	...
	os.execute("netsh advfirewall firewall add rule name='Infocyte Host Isolation' dir=in action=allow protocol=ANY profile=ALL remoteip=" .. iplist)
	os.execute("netsh advfirewall firewall add rule name='Infocyte Host Isolation' dir=out action=allow protocol=ANY profile=ALL remoteip=" .. iplist)
...
elseif  hunt.env.has_sh() then
	...
	os.execute("rm /opt/iptables-bkup")
	handle = assert(io.popen('iptables-save > /opt/iptables-bkup', 'r'))
	...
	hunt.log("Allowing Infocyte API IP: " .. list_to_string(infocyte_ips))
	for _, ip in pairs(hunt.net.api_ipv4()) do
	  os.execute("iptables -I INPUT -s " .. ip .. " -j ACCEPT")
	end

	hunt.log("Allowing Zoom IPs: " .. list_to_string(infocyte_ips))
